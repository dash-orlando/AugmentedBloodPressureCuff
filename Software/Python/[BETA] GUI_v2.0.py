...
*   - ADDED   : Choose mode [SIM vs REC]
*   - FIXED   : Clean up code
*   - MODIFIED: Can now pass list of addresses instead
*               of hardcoding them into the class.
*
* KNOWN ISSUES:
...
# ------------------------------------------------------------------------
    ...
    def launch_win( self, prompt ):
        ...
        self.mode = self.app.getOptionBox( "Mode  \t" )                 # Get Mode
...
# ------------------------------------------------------------------------
    ...
    def start_bpc( self, prompt ):
        ...
        if( prompt == "Stop" ):
            ...
            cmd = "python pressureDialGauge_GUI.py --directory %s --destination %s --stethoscope %s" %( self.cty, self.dst, self.stt )
