...
from    bluetoothProtocol_teensy32  import findDevices          # Scan for BT devices
...
class MyWindow(QtGui.QMainWindow):
    ...
    def connectStethoscope(self, address):
        ...
        self.ui.CommandLabel.setText( address + " Selected" )
    ...
    def scan_rfObject(self):
        ...
        available = []
        q = Queue()
        p = Process( target=findDevices, args=(q,) )
        p.start()
        BT_name = q.get()
        BT_address = q.get()
        for i in range( 0, (len(BT_name) - 1) ):
            available.append( (BT_name[i], BT_address[i]) )
